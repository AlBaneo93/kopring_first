...
@Component
@Slf4j
class JwtUtil(val userService: UserService) {
    ...
    fun validAccess(token: String): Boolean {
        return tokenValidate(token, access_exp)
    }

    fun validRefresh(token: String): Boolean {
        return tokenValidate(token, refresh_exp)
    }
    ...
    private fun tokenValidate(token: String, expTime: Long): Boolean {
